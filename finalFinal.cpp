...
struct Player {
    ...
    int scoreValue = 0, lifes = 3, hitCounter = -1, deathDealy = 0, FinalScore = 0;
...
int main()
{
    ...
    if (start && !Exit) {
        ...
        Text lives;
        lives.setFont(font);
        lives.setString("x" + std::to_string(sonic.lifes));
        ...
        while (window.isOpen())
        {
               ...
               // cout << sonic.hitCounter << '\n';
                fa(i, 0, 180) {
                    if (sonic.hitCounter == -1) {
                        if (spikes[i].SpikeSprite.getGlobalBounds().intersects(sonic.PlayerColl.getGlobalBounds())) {
                            if (!sonic.hitLeft && !sonic.hitRight && sonic.hitCounter == -1) sonic.lifes--;
                ...
                if (Boss.FightStartLevel1 && Level == 1 && Boss.lives > 0) {
                    ...
                    fa(i, 0, 5) {
                        ...
                        if (skyBullets[i].BulletColl.getGlobalBounds().intersects(sonic.PlayerColl.getGlobalBounds())) {
                            skyBullets[i].SkyBulletsSprite.setScale(0, 0);
                            skyBullets[i].BulletColl.setScale(0, 0);
                            if (sonic.hitCounter == -1) sonic.lifes--;
                            sonic.hitCounter = 50;
                            SonicHitAudio.play();
                        }
                    }
                ...
                if (sonic.PlayerColl.getGlobalBounds().intersects(Boss.HitBox.getGlobalBounds()) && Boss.lives > 0 && Level == 1) {
                    if (sonic.Velocity.y >= 0) {
                        if (!sonic.hitLeft && !sonic.hitRight) sonic.lifes--;
            ...
            fa(i, 0, 2) window.draw(Red_coins[i].CoinSprite);
            ...
            fa(i, 0, 320) window.draw(coins[i].CoinSprite);
